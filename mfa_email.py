...
def normalize_gmail_app_password(raw: str) -> str:
    """Gmail app passwords are 16 chars; spaces in input are optional."""
...
def validate_gmail_app_password(pwd: str) -> str | None:
    ...
    if not pwd:
        return "Gmail app password is empty."
    if len(pwd) != 16:
        return (
            f"Gmail app password should be 16 characters "
            f"(got {len(pwd)}). Remove quotes, spaces, or stray characters."
        )
    if not pwd.isalnum():
        return "Gmail app password should be letters and digits only."
    return None
...
def _delete_messages(mail, uids: list) -> None:
    """Move the given messages to Trash so the inbox isn't crowded with codes.

    Uses Gmail's X-GM-LABELS extension (adds \\Trash -> moves to Trash, auto-
    purged in ~30 days). Falls back to the standard \\Deleted flag + EXPUNGE for
    non-Gmail servers.
    """
    if not uids:
        return
    for uid in uids:
        moved = False
        try:
            typ, _ = mail.uid("STORE", uid, "+X-GM-LABELS", "\\Trash")
            moved = (typ == "OK")
        except Exception:
            moved = False
        if not moved:
    ...
    try:
        ...
        pass
    logger.info("Deleted %d USCIS verification email(s) from the inbox.", len(uids))
...
def _try_read_code(
    ...
    latest_code = None
    latest_uid = 0
    mfa_uids: list = []   # UIDs of MFA verification emails from this login
    ...
    try:
        ...
        if "AUTHENTICATIONFAILED" in err:
            raise RuntimeError(
                ...
                "App passwords). Use the same Gmail as the USCIS account email."
            ) from exc
        raise
    mail.select("INBOX")
    ...
    # Gmail: prefer X-GM-RAW for a precise sender/time search.
    try:
        typ, data = mail.uid(
            "SEARCH",
            None,
    ...
    for uid in uids[-30:]:
        ...
        if msg_date and msg_date < since:
            continue
        # A fresh USCIS verification email — eligible for cleanup.
        mfa_uids.append(uid)
    ...
    # Only delete once we actually have the code, so a parse miss never loses it.
    if delete_after_read and latest_code:
        _delete_messages(mail, mfa_uids)
...
def fetch_verification_code(
    ...
    When delete_after_read is True, the verification email is moved to Trash
    once the code has been read, so the inbox isn't crowded with codes.
    """
    since = None
    start = time.time()
    code = None
    ...
    while time.time() - start < MAX_WAIT_SEC:
        try:
            code = _try_read_code(email_addr, app_password, since, delete_after_read)
    ...
    raise TimeoutError("Timed out waiting for USCIS verification email")
